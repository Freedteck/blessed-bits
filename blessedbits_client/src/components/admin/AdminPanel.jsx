...
const AdminPanel = ({
  ...
  const [configValues, setConfigValues] = useState({
    ...
    blessPerSui: 0,
    dailyRewardsBase: 0,
    stakingRewardPercent: 0,
  });
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);

  const { stats, isPending, refetch } = usePlatformStats(
    platformStateId,
    platformConfigId
  );
  ...
  useEffect(() => {
    if (stats) {
      setConfigValues({
        dailyCashback: stats.dailyCashbackAmount,
        minPurchase: stats.minPurchaseAmount,
        blessPerSui: stats.blessPerSui,
        dailyRewardsBase: stats.dailyRewardsBase,
        stakingRewardPercent: stats.stakingRewardPercent,
      });
    }
  }, [stats]);
